from fastapi import Depends, FastAPI, UploadFile, File, Form, Query, HTTPException, Response
...
async def apply_style_transfer(
    ...
    try:
        content = await load_image(content_file)
        style = await load_image(style_file)
        result = custom_style_transfer(content, style)
        return Response(content=image_to_bytes(result, ".jpg"), media_type="image/jpeg")
    except Exception as e:
